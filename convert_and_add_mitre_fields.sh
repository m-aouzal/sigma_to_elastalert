...
rm -f "$FAIL_LIST"
# --- Post-Processing: Update Converted Rules with MITRE Fields ---
echo "Checking for rule YAML files in ${OUTPUT_DIR}..."
if ! find "$OUTPUT_DIR" -type f -name "*.yml" | grep -q .; then
  echo "Error: No rule YAML files found in ${OUTPUT_DIR}. Aborting update."
  exit 1
fi

echo "Calling update_converted_sigma.py to update converted rules with MITRE fields..."
python3 update_converted_sigma.py --converted_dir "$OUTPUT_DIR" --sigma_dir "./sigma/rules/windows/" --output_dir "./final_rules" --default_risk Medium
if [ $? -eq 0 ]; then
    echo "Post-processing completed successfully."
    echo "Moving final rules directory to output directory..."
    rm -rf "$OUTPUT_DIR"
    mv "./final_rules" "$OUTPUT_DIR"
    echo "Final rules have been moved to ${OUTPUT_DIR}."
else
    echo "Post-processing failed. Please check the temporary directory ${OUTPUT_DIR} for details."
    exit 1
fi
